...
META-IR Predictor for meta-learning-based recommendation in imbalanced regression tasks.
...
def run_predictions(X_path, model_dir):
    model_paths = {
        'independent': os.path.join(model_dir, "meta_independent.pkl"),
        'model_first': os.path.join(model_dir, "meta_model_first.pkl"),
        'strategy_first': os.path.join(model_dir, "meta_strategy_first.pkl"),
    }
    meta_pred = META_IR_Predictor(model_paths)
    X_new = pd.read_csv(X_path).dropna()
    if X_new.shape[1] > 1:
        X_new = X_new.drop(X_new.columns[0], axis=1)

    for mode in ['independent', 'model_first', 'strategy_first']:
        preds = meta_pred.predict(X_new, mode=mode)
        out_path = os.path.join(model_dir, f"recommendation_{mode}.csv")
        preds.to_csv(out_path, index=False)
